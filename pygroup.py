...
@pygroup_app.route('/threegear', defaults={'n1':15,'n2':20,'n3':18})
...
'''
    ...
    # 若 template 檔案名稱與位於外部 templates 目錄中的檔案同名, 則取外部的 template
   # return render_template('g1index.html', user=user)
@pygroup_app.route('/canvas', defaults={'id': 'plotarea', 'height': 800, 'm': 10})
...
def listfiles():
    ...
    return file_string
@pygroup_app.route('/get/<filename>')
def get(filename):
    # 先檢查使用者是否處於登入狀態, 若尚未登入則跳轉到登入畫面
    if not session.get('login_email'):
        #abort(401)
